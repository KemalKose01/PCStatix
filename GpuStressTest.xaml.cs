...

{
    ...
    public partial class GpuStressTest : UserControl
    {
        ...
        private float _currentGpuTemp; // sınıf seviyesinde değişken

        private void UpdateSensorss()
        {
                         ...
                         || h.HardwareType == HardwareType.GpuIntel))
            {
                ...
                foreach (var sensor in hardware.Sensors)
                {
                    if (sensor.SensorType == SensorType.Temperature)
                    {
                        _currentGpuTemp = sensor.Value ?? 0; // değişkene ata
                        TxtCurrentTemp.Text = $"{Math.Round(_currentGpuTemp, 1)}°C";
                    }
                }
            }
        }
        ...
        private void StopTest()
        {
            ...
            if (_temps.Count > 0 && _loads.Count > 0)
            {
               float maxT = _temps.Max();
                ...
                string gpuName = _computer.Hardware
                .FirstOrDefault(h => h.HardwareType == HardwareType.GpuNvidia
                                 || h.HardwareType == HardwareType.GpuAmd
                                  || h.HardwareType == HardwareType.GpuIntel)?.Name
                ?? "Bilinmeyen GPU";

                using (var context = new PcStatixContext())
                {
                    
                    context.SaveGpuData(gpuName, (int)maxT);
                }
                using (var context = new PcStatixContext())
                {
                    double ortalama=context.GetAverageGpuTemp(gpuName);
                    MessageBox.Show($"CPU {gpuName}\nOrtalama: {ortalama:0.0}°C");

                }
                using (var context = new PcStatixContext())
                {
                    string health = context.GpuHealth(context.GetAverageCpuTemp(gpuName), (int)maxT);
                    MessageBox.Show($"GPU {gpuName}\n{health}\n{(int)maxT}");
                }



            }
            // GPU adını güvenli şekilde al
